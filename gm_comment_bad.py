...
with open(out_file, "w") as csvfile:
    ...
    writer.writerow(['p_name','brand','commentcount','source','link','badcomment'])
for x in range(len(id_list)):
    ...
    for i in range(1,31):
        ...
        url="https://ss.gome.com.cn/item/v1/prdevajsonp/appraiseNew/"+str(id)+"/"+str(i)+"/bad/1/1557/flag/appraise"
                ...
                try:
                    coll.insert(
                        {'ProductID': str(id), 'p_name': p_Name, 'brand': brand, 'commentcount': str(CommentCount),
                         'source': source, 'link': product_url, 'badcomment': comment_text, 'X_type': X_type,
